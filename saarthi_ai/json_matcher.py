...
class JSONOpportunityMatcher:
    ...
    def is_eligible(self, student: Dict[str, Any], opportunity: Dict[str, Any]) -> bool:
        ...
        - gender matches OR is "Any"
        - education_level matches OR is "Any"
        - eligible_fields matches OR is "Any"
        - target_year matches OR is "Any"
        - at least one goal matches goal_tags
        
        Args:
            student: Student data dict with keys: gender, education_level, field, year, 
                     age, skills, institution_type, background, goals
        ...
        try:
            ...
            # Gender check (CRITICAL - must match exactly or be "Any")
            opp_gender = eligibility.get('gender', 'Any')
            student_gender = student.get('gender', '').strip()
            
            if opp_gender and opp_gender != 'Any':
                # If opportunity requires specific gender and student didn't provide gender, exclude
                if not student_gender:
                    return False
                
                # Normalize and compare (case-insensitive)
                student_gender_lower = student_gender.lower()
                opp_gender_lower = opp_gender.lower()
                
                # If opportunity requires Female and student is not Female, exclude
                if opp_gender_lower == 'female' and student_gender_lower != 'female':
                    return False
                # If opportunity requires Male and student is not Male, exclude  
                elif opp_gender_lower == 'male' and student_gender_lower != 'male':
                    return False
            ...
            # Goals check (CRITICAL - at least one goal must match)
            opportunity_goals = opportunity.get('goal_tags', [])
            student_goals = student.get('goals', [])
            
            if opportunity_goals and isinstance(opportunity_goals, list) and len(opportunity_goals) > 0:
                if not student_goals or not isinstance(student_goals, list) or len(student_goals) == 0:
                    # If opportunity has goal tags but student has no goals, exclude
                    return False
                
                # Check if at least one student goal matches opportunity goal_tags
                # Normalize for comparison (case-insensitive)
                student_goals_lower = [g.lower().strip() for g in student_goals]
                opportunity_goals_lower = [g.lower().strip() for g in opportunity_goals]
                
                has_matching_goal = any(
                    student_goal in opportunity_goals_lower 
                    for student_goal in student_goals_lower
                )
        ...
        for opportunity in self.opportunities:
            if self.is_eligible(student, opportunity):
                ...
                formatted_opp = {
                    ...
                    'eligibility': opportunity.get('eligibility', {}),
                    'goal_tags': opportunity.get('goal_tags', []),
