            ...
            matches_data = [
                {
                    ...
                    'organization_description': opp.get('organization_description', ''),
                    'location': opp.get('location', ''),
                    'work_mode': opp.get('work_mode', ''),
                    'duration': opp.get('duration', ''),
                    'start_date': opp.get('start_date', ''),
                    'stipend': opp.get('stipend', 'Not specified'),
                    'perks': opp.get('perks', []),
                    'apply_link': opp.get('apply_link', ''),
                    'application_deadline': opp.get('application_deadline', 'Varies'),
                    'roles_responsibilities': opp.get('roles_responsibilities', []),
...
def _normalize_field_of_study(field: str) -> str:
    ...
    # Computer Science (can be separate or part of Engineering)
    ...
    # Commerce variations
    if any(term in field_lower for term in ['commerce', 'bcom', 'b.com']):
        return 'Commerce'
    
    # Economics variations
    if any(term in field_lower for term in ['economics', 'economic']):
        return 'Economics'
    
    # Finance variations
    if any(term in field_lower for term in ['finance', 'financial', 'accounting']):
        return 'Finance'
